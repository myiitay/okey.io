...
function Page101Content() {
    ...
    const [theme, setTheme] = useState<'crimson' | 'ruby' | 'fire' | 'rose'>('crimson');
    ...
    useEffect(() => {
        ...
        const savedTheme = localStorage.getItem("okey_101_theme") as any;
    ...
    useEffect(() => {
        if (isLoaded) {
            ...
            localStorage.setItem("okey_101_theme", theme);
    ...
    useEffect(() => {
        ...
        if (joinCode && isLoaded && !isRejoining) {
    ...
    const handleCreate = () => {
        ...
        socket.emit("createRoom", { name: nickname, avatar: avatars[avatarId], frameId: frameId, gameMode: '101' });
    };
    ...
    const themeConfigs = {
        crimson: {
            bg: "bg-[#1a0505]",
            gradient: "from-[#3d0a0a] via-[#5c0f0f] to-[#1a0505]",
            mesh: "rgba(220,38,38,0.3)",
            accent: "text-red-600",
        ...
        ruby: {
            bg: "bg-[#2d0a0f]",
            gradient: "from-[#5c1420] via-[#7f1d1d] to-[#2d0a0f]",
            mesh: "rgba(239,68,68,0.2)",
        ...
        fire: {
            bg: "bg-[#1c0b00]",
            gradient: "from-[#451a03] via-[#7c2d12] to-[#1c0b00]",
            mesh: "rgba(249,115,22,0.25)",
        ...
        rose: {
            bg: "bg-[#1f0a14]",
            gradient: "from-[#4c0519] via-[#881337] to-[#1f0a14]",
            mesh: "rgba(244,63,94,0.2)",
            accent: "text-pink-900",
            button: "from-pink-600 to-rose-700",
            ...
            >
                ...
                <div className="absolute top-6 left-6 z-[100]">
                    <button
                        ...
                        className="relative group/stdbtn transform hover:scale-110 active:scale-95 transition-all duration-300"
                    >
                        <div className="relative overflow-hidden rounded-[2rem] p-1.5 shadow-[0_20px_50px_rgba(79,70,229,0.4)]">
                            ...
                            <div className="relative bg-[#0f172a] rounded-[1.8rem] px-10 py-5 flex flex-col items-center justify-center border-2 border-indigo-500/50 group-hover:bg-transparent transition-colors">
                                <span className="text-white font-black text-4xl tracking-tighter drop-shadow-lg leading-none">STANDART</span>
                                <span className="text-indigo-500 group-hover:text-white font-bold text-[10px] tracking-[0.3em] uppercase mt-1">MODU</span>
                            </div>
                        </div>
                    </button>
                </div>
                ...
                <div className="z-10 w-full max-w-xl px-4 relative flex flex-col items-center">
                    ...
                    <div className="mb-4 relative group cursor-default">
                        <h1 className="text-6xl font-black text-white tracking-tighter drop-shadow-[0_8px_0_rgba(0,0,0,0.2)] transform rotate-[-3deg] hover:rotate-[3deg] transition-transform duration-300">
                            OKEY<span className="text-red-400">101</span>
                        </h1>
                                    ...
                                    <div className="flex flex-col items-center mb-6">
                                        <div className="relative">
                                            <div className="relative group/avatar">
                                                <div className={`w-24 h-24 bg-white shadow-xl flex items-center justify-center text-5xl mb-3 relative hover:scale-110 transition-transform cursor-pointer overflow-visible ring-4 ${theme === 'fire' ? 'ring-orange-900/20' : 'ring-red-500/10'} ${frames.find(f => f.id === frameId)?.style || 'rounded-full border-white'}`}>
                                                    {avatars[avatarId]}
                                                </div>
                                                <div className="absolute -bottom-2 -right-2 bg-red-400 text-white p-2 rounded-full border-4 border-white shadow-sm text-sm font-bold cursor-pointer hover:scale-110 transition-transform z-10">
                                                    ✏️
                                                </div>
                                                    ...
                                                    <div className="grid grid-cols-6 gap-2 max-h-48 overflow-y-auto custom-scrollbar p-1">
                                                        {avatars.map((av, i) => (
                                                            <button
                                                                ...
                                                                className={`w-12 h-12 flex items-center justify-center text-3xl rounded-xl hover:bg-gray-100 transition-colors ${avatarId === i ? 'bg-red-50 ring-2 ring-red-400 shadow-inner' : ''}`}
                                                            >
                                                                {av}
                                                            </button>
                                                        ))}
                                                        ...
                                                        <div className="flex gap-6 p-4 overflow-x-auto custom-scrollbar whitespace-nowrap min-h-[100px] items-center">
                                                            {frames.map((f) => (
                                                                <button
                                                                    ...
                                                                    className={`w-12 h-12 border-2 transition-all hover:scale-110 flex-shrink-0 flex items-center justify-center relative ${frameId === f.id ? 'ring-4 ring-red-500/30 scale-125 z-10' : ''} ${f.style}`}
                                                                    style={{ borderColor: f.id === 'none' ? '#eee' : 'transparent', backgroundColor: 'white' }}
                                                                    title={f.name}
                                                                >
                                                                    {f.id === 'none' ? '❌' : ''}
                                                                    {frameId === f.id && <div className="absolute -top-1 -right-1 bg-red-500 w-3 h-3 rounded-full border border-white shadow-sm z-20"></div>}
                                                                </button>
                                                            ))}
                                                        </div>
                                                    </div>
                                            ...
                                            <div
                                                ...
                                                className="absolute top-1/2 -right-40 -translate-y-1/2 bg-red-600 text-white p-3 rounded-2xl border-4 border-white shadow-xl text-2xl font-bold cursor-pointer hover:scale-110 hover:rotate-12 transition-all flex items-center justify-center z-10"
                                                title="Market"
                                            >
                                                🛒
                                            </div>
                                            ...
                                            <div className="space-y-6 animate-scaleIn">
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="flex flex-col gap-3">
                                                        <input
                                                            ...
                                                            className={`w-full bg-white border-4 border-gray-100 focus:border-red-500 rounded-2xl px-4 py-4 text-center font-mono text-3xl font-black ${currentTheme.accent} outline-none uppercase placeholder-gray-100 transition-all`}
                                                        />
                                                        <button
                                                            onClick={handleJoin}
                                                            disabled={!isConnected}
                                                ...
                                                <div className="space-y-3 max-h-[180px] overflow-y-auto custom-scrollbar pr-2">
                                                    {rooms.length === 0 ? (
                                                        ...
                                                        rooms.map((room) => (
                                                            <div key={room.id} className="bg-white border-2 border-red-100 rounded-2xl p-3 flex items-center justify-between hover:border-red-300 hover:shadow-md transition-all group">
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        className={`px-5 py-2.5 rounded-xl font-bold text-white text-sm shadow-sm transition-all active:scale-95 ${room.count >= room.max || room.status === 'Playing' ? 'bg-gray-300 cursor-not-allowed' : 'bg-red-500 hover:bg-red-600 hover:shadow-lg'}`}
                                                                    >
                                                                        KATIL
                                                                    </button>
                                                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-4">
                                        <h3 className="text-lg font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                            <span className="w-8 h-8 bg-red-100 text-red-600 rounded-lg flex items-center justify-center">🎨</span>
                                            Masa Teması
                                        </h3>
                                            ...
                                            {[
                                                ...
                                                <button
                                                    ...
                                                    className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all hover:scale-105 relative
                                                        ${tableTheme === t.id ? 'border-red-500 bg-red-50 shadow-inner' : 'border-gray-100 hover:bg-gray-50'}`}
                                                >
...
export default function Page101() {
    return (
        <Suspense fallback={<div className="h-screen w-screen bg-[#1a0505] flex items-center justify-center text-white font-black text-2xl animate-pulse">YÜKLENIYOR...</div>}>
            <Page101Content />
        </Suspense>
    );
}
