...
class HandValidator {
    ...
    static removeTiles(source, toRemove) {
    ...
    // ... Helper finders
    static findPossibleGroups(start, pool) {
        ...
        if (start.isJoker)
            return []; // Should have sorted jokers to end, so start is rarely joker unless all jokers.
        ...
        const combine = (currentSet, index) => {
            ...
            for (let i = index; i < candidates.length; i++) {
                ...
                if (!next.isJoker && realTiles.some(x => x.color === next.color))
                    continue;
                // Add
    ...
    static findPossibleRuns(start, pool) {
        if (start.isJoker)
            return [];
        const results = [];
...
exports.HandValidator = HandValidator;
